...
{   
    ...
    void* memory2[ALLOCATE_COUNT];
    cout << "use malloc free time" << endl;
    cout << "--------------------" << endl;
    auto start = std::chrono::steady_clock::now();
    for (int i = 0; i < ALLOCATE_COUNT; i++) 
    {
        memory[i] = malloc(sizeof(10));
    }
    ...
    for (int i = 0; i < ALLOCATE_COUNT; i++) 
    {
        memory2[i] = pool.malloc(sizeof(10));
    }
    end = std::chrono::steady_clock::now();
    cout << "the time cost " << std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() << "us" << endl;

    showInfo(&pool, "创建10000个10字节的内存块");

    start = std::chrono::steady_clock::now();
    for (int i = 0; i < ALLOCATE_COUNT - 5000; i++) 
    {
        pool.freeMemory(memory2[i]);
    }
    showInfo(&pool, "销毁内存池");
    pool.destroyPool();
